...
- 这是个人 fork，不是上游保守维护分支。允许为了长期维护性主动、大幅、连贯地重构；不要因为“改动大”而回避正确的架构拆分。
- Agent 必须有 **owner 意识**：发现潜在问题就追到根因，能顺手修就修，不能修也要写入 TODO/ADR；不要只完成表面需求。
- 对屎山 **0 容忍**：禁止继续堆叠 god object、隐式全局状态、重复逻辑、临时补丁、类型抑制、浏览器原生 prompt/alert 等债务。新增代码必须让边界更清晰，而不是让未来更难改。
- 重构优先级高于短期 UI 小修。除非是阻塞使用的 bug，否则先拆边界、补测试、清死代码、建立可验证的架构护栏，再做体验层 polish。
- **不要在 PR 中请求合并 `AGENTS.md`**。这是个人 fork 的本地项目知识库，包含 fork 特有的工作原则、代码地图和维护约定。与上游合并时应排除此文件。
...
│   ├── archive/              # 封存遗留代码（如 pre-1.0 Neo4j 迁移），活跃路径禁止引用
│   ├── db/                   # 数据层 (ORM + 服务 + 迁移)
...
| Neo4j legacy archive | 封存代码 | `backend/archive/neo4j_legacy/` | 约2600 | ⚠️ pre-1.0 Neo4j 迁移参考，活跃路径禁止引用 |
...
- Neo4j 遗留实现已封存在 `backend/archive/neo4j_legacy/`，只作为 pre-1.0 手动迁移参考。活跃 `db/`/`api/`/MCP 路径禁止引用。
